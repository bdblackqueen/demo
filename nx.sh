...
clear
    ...
    echo -e "             ⊂(___)づ  ⋅˚₊‧ ଳ ‧₊˚ ⋅"              
    echo -e "\n ${g}[${n}${KER}${g}] ${c}Exiting ${g}NX Banner \033[1;36m"
    echo
    cd "$HOME"
    rm -rf NX
    exit 0
...
mv $HOME/NX/files/report $HOME/.NX-simu
mv $HOME/NX/files/chat.sh /data/data/com.termux/files/usr/bin/chat
...
if [ -f "$simu" ]; then
	...
	cp $HOME/NX/files/colors.properties "$ds"
fi
cp $HOME/NX/files/ASCII-Shadow.flf $PREFIX/share/figlet/
mv $HOME/NX/files/remove /data/data/com.termux/files/usr/bin/
...
donotchange() {
    ...
    VERSION="$D1/dx.txt"
    INPUT_FILE="$HOME/NX/files/.zshrc"
    THEME_INPUT="$HOME/NX/files/.NX.zsh-theme"
    OUTPUT_ZSHRC="$HOME/.zshrc"
    OUTPUT_THEME="$HOME/.oh-my-zsh/themes/NX.zsh-theme"
...
echo -e "${b}╭════════════════════════⊷"
echo -e "${b}┃ ${g}[${n}ム${g}] ᴛɢ: ${y}t.me/NX_Termux"
echo -e "${b}╰════════════════════════⊷"
        else
echo -e "${b}╭══════════════════════════⊷"
echo -e "${b}┃ ${g}[${n}ム${g}] ᴛɢ: ${y}t.me/imran"
echo -e "${b}╰══════════════════════════⊷"
        fi
echo
echo -e "${b}╭══ ${g}〄 ${y}𝗡𝗫 ${g}〄"
echo -e "${b}┃❁ ${g}ᴄʀᴇᴀᴛᴏʀ: ${y}ᴅx-ᴄᴏᴅᴇx"
echo -e "${b}┃❁ ${g}𝗼𝘄𝗻𝗮𝗿: ${y}𝗔𝗹 𝗜𝗺𝗿𝗮𝗻"
    ...
    if [[ "$input" == $'\e' ]]; then
        ...
        case ${options[$selected]} in
            ...
            "Premium")
                ...
                sleep 1
                xdg-open "https://t.me/NX_AL_IMRAN_S"
                cd "$HOME"
            	rm -rf NX
                exit 0
                ;;
